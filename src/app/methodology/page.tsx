  ...
  return (
    <article className="max-w-3xl">
      ...
      <section className="mt-10 space-y-4 text-sm leading-7 text-muted" id="demo">
        ...
        <p>
          The shipping dataset is a demo. Analysts are invented. Firm names are familiar labels so the product can be searched; the hit rates attached to them are simulated. Each fictional analyst is given a skill level so the leaderboard has a spread. None of it is a track record, a forecast, or a description of anyone’s research.
        </p>
        <p>
          Sample prices follow split-adjusted trading history for that calendar date, plus a small seeded wiggle. They are not a live quote. The names with a split in this window —{" "}
          {Object.entries(SPLIT_ADJUSTED)
            .map(([symbol, info]) => `${symbol} (${info.split})`)
            .join(", ")}
          {" "}— use the post-split scale for the whole path, including dates before the split, so a window that crosses the split does not print a fake crash. The price at the call, the target, and the later prints all use that same scale.
        </p>
        <p>
